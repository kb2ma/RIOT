...
#define TINYDTLS_LOG (DTLS_LOG_EMERG)
                         ...
                         unsigned char *result, size_t result_length)
{
    ...
    psk_params_t psk;
    tlsman_get_psk_credentials(&psk);
    switch(type) {
        case DTLS_PSK_HINT:
            DEBUG("psk hint request\n");
            /* server: unused */
            /*
            if (psk.hint_len > result_length) {
                DEBUG("psk hint too big for result buffer\n");
                return -1;
            }

            if (psk.hint_len == 0) {
                DEBUG("no psk hint defined\n");
                return -1;
            }
        ...
        case DTLS_PSK_IDENTITY:
            ...
            if (psk.id_len > result_length) {
                DEBUG("psk id too big for result buffer\n");
                return -1;
            }

            if (psk.id_len == 0) {
                DEBUG("no psk id defined\n");
                return -1;
            }
        ...
        case DTLS_PSK_KEY:
            ...
            if (psk.key_len > result_length) {
                DEBUG("psk key too big for result buffer\n");
                return -1;
            }

            if (psk.key_len == 0) {
                DEBUG("no psk key defined\n");
                return -1;
            }
                          ...
                          const dtls_ecdsa_key_t **result)
{
    ...
    if (tlsman_get_ecdsa_credentials(&ecdsa) < 0) {
    ...
    *result->priv_key = ecdsa.priv_key;
    *result->pub_key_x = ecdsa.pub_key_x;
    *result->pub_key_y = ecdsa.pub_key_y;
    //*result->curve = ecdsa.curve;
    return 0;
}
                             ...
                             const unsigned char *other_pub_y, size_t key_size)
{
    ...
    if (memcmp(ecdsa.pub_key_x, other_pub_x, key_size) != 0) {
        DEBUG("ECDSA public key x does not match\n");
        return -1;
    }

    if (memcmp(ecdsa.pub_key_y, other_pub_y, key_size) != 0) {
        DEBUG("ECDSA public key y does not match\n");
        return -1;
    }
                                ...
                                sock_dtls_session_t *remote)
{
    ...
    while (!mbox_try_get(&sock->mbox, &msg) ||
        ...
        ssize_t res = sock_udp_recv(sock->udp_sock, rcv_buffer,
                                        sizeof(rcv_buffer),
        ...
        if (res >= 0) {
            ...
            DEBUG("Error receiving handshake messages: %d\n", res);
            return res;
        }
    }
    return 0;
...
{
    dtls_init();
    dtls_set_log_level(TINYDTLS_LOG);
}
